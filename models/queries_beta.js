...
const Beta5 = `
    ...
    group_1 AS (
        ...
        FROM ranked_published_root_contents
        WHERE
            published_at > NOW() - INTERVAL '4 hours'
            AND tabcoins > 11
        ORDER BY
            published_at DESC
        LIMIT 10
    ...
    group_2 AS (
        ...
        SELECT
            *,
            2 as rank_group
        FROM ranked_published_root_contents
        WHERE ((published_at > NOW() - INTERVAL '4 hours' AND tabcoins > 6)
            OR (published_at > NOW() - INTERVAL '9 hours' AND tabcoins > 11))
            AND id NOT IN (SELECT id FROM group_1)
        ORDER BY
            rank_group,
            published_at DESC
    ...
    group_3 AS (
        ...
        SELECT
            *,
            3 as rank_group
        FROM ranked_published_root_contents
        WHERE ((published_at > NOW() - INTERVAL '4 hours' AND tabcoins > 3)
            OR (published_at > NOW() - INTERVAL '9 hours' AND tabcoins > 6)
            OR (published_at > NOW() - INTERVAL '18 hours' AND tabcoins > 11))
            AND id NOT IN (SELECT id FROM group_2)
        ORDER BY
            rank_group,
            published_at DESC
    ...
    group_4 AS (
        ...
        FROM ranked_published_root_contents
        WHERE
            published_at > NOW() - INTERVAL '9 hours'
            AND id NOT IN (SELECT id FROM group_3)
        ORDER BY
            published_at DESC
        LIMIT 5)
    ...
    group_5 AS (
        (SELECT
            *,
            5 as rank_group
        FROM ranked_published_root_contents
        WHERE ((published_at > NOW() - INTERVAL '72 hours' AND tabcoins > 6)
            OR (published_at > NOW() - INTERVAL '18 hours' AND tabcoins > 2))
            AND id NOT IN (SELECT id FROM group_4)
        ORDER BY
            published_at DESC)
        UNION ALL
