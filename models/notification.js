...
async function sendReplyEmailToParentUser(createdContent) {
  ...
  if (parentContent.owner_id !== secureCreatedContent.owner_id) {
    ...
    const childContendUrl = getContentUrl(secureCreatedContent);
...
async function sendUserDisabled({ eventId, user }) {
  const { html, text } = FirewallEmail({
    sideEffectLine: 'Identificamos que você está tentando criar muitas contas, então a sua conta foi desabilitada.',
    eventId: eventId,
  ...
  await email.send({
    ...
    subject: 'Sua conta foi desabilitada',
    html,
    text,
  });
}
...
function getFirewallDeletedContentLine(contents) {
  ...
  if (isRootContent) {
    if (contents.length > 1) {
      beforeTitles = 'muitas publicações, então as suas publicações';
      afterTitles = 'foram removidas';
    } else {
      beforeTitles = 'muitas publicações, então a sua publicação';
      afterTitles = 'foi removida';
    }
  } else {
    if (contents.length > 1) {
      beforeTitles = 'muitos comentários, então os seus comentários de IDs';
      afterTitles = 'foram removidos';
    } else {
      beforeTitles = 'muitos comentários, então o seu comentário de ID';
