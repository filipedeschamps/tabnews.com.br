  ...
  return (
    <div className="flex flex-col space-y-4">
      {posts.map((post, index) => {
        return (
          <Link href="#">
            <a className="flex flex-col p-6 space-y-2 bg-white rounded-md shadow-sm ">
              <div className="text-base text-gray-700">
                <span className="font-semibold">{post.title}:</span>{" "}
                <span>{post.text}</span>
              </div>

              <div className="flex items-center space-x-3 text-sm tracking-tight">
                <div className="flex items-center font-bold text-yellow-500">
                  <RiMoneyDollarCircleFill className="w-3 h-3 mr-1" />
                  {post.coins}
                </div>
                <div className="flex items-center font-bold text-gray-400">
                  <BsFillChatFill className="w-3 h-3 mr-1" />
                  {post.coins}
                </div>
