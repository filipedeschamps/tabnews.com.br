...
export default function copyToClipboardPlugin() {
  return {
    viewerEffect({ markdownBody }) {
      const preElements = markdownBody.querySelectorAll('pre');
      preElements.forEach((pre) => {
        const copyToClipboardButton = document.createElement('button');
        copyToClipboardButton.innerHTML = copyIconSvg;
        ...
        copyToClipboardButton.onclick = () =>
          copyToClipboard(pre.querySelector('code').innerText, copyToClipboardButton);
        pre.appendChild(copyToClipboardButton);
      });
    },
  };
}
