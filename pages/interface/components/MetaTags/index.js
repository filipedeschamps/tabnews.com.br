...
export default function MetaTags({ metadata }) {
  ...
  if (metadata?.title && !metadata.title.endsWith(' · TabNews')) {
    metadata.title = `${metadata?.title} · TabNews`;
  }

  let updatedMetadata = { ...defaultMetadata, ...metadata };

  const { type, title, description, image, url, noIndex, author, published_time, modified_time } = updatedMetadata;
