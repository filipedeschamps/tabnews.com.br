...
export default function SearchRecents({ query, searchRuntime, contentListFound, pagination }) {
  return (
    <>
      <DefaultLayout
        metadata={{
          title: `Página ${pagination.currentPage || 1} · Buscar ${query}`,
          description: `Busca por "${query}" no TabNews ordenadas pelas mais recentes.`,
        }}>
        ...
        <ContentList
          ...
          paginationBasePath={`/buscar/recentes`}
        />
      </DefaultLayout>
    </>
  );
...
export async function getServerSideProps(context) {
  ...
  try {
    const results = await search.findAll({ searchTerm: context.query.q, page: context.params.page });
