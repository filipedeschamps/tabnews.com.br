import ContentListVote from '@/ContentListVote';
import {
  ActionList,
  ActionMenu,
  Box,
    ...
    <>
      <DefaultLayout metadata={{ title: `${userFound.username}` }}>
        ...
        <ContentListVote
          ...
          paginationBasePath={`/${userFound.username}/votos/pagina`}
          emptyStateProps={{
            isLoading: isLoading,
            title: 'Nenhum conteúdo encontrado',
            description: `${isAuthenticatedUser ? 'Você' : userFound.username} ainda não votou em nenhuma publicação.`,
            icon: FaUser,
            action: isAuthenticatedUser && {
              text: 'Votar em algum conteúdo',
              onClick: () => push('/'),
            },
          }}
        />
...
export const getStaticProps = getStaticPropsRevalidate(async (context) => {
  ...
  try {
    ...
    results = await content.findUserVotes({
  ...
  const secureContentListFound = authorization.filterOutput(userTryingToGet, 'read:content:votes', contentListFound);
