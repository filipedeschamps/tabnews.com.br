...
describe('PATCH /api/v1/mfa/totp/enable', () => {
  ...
  describe('Default user', () => {
    test('With valid session and necessary features', async () => {
      ...
      expect(responseBody.message).toBe('O duplo fator de autenticação foi configurado com sucesso.');
    });
    ...
    test('With valid session and necessary features but user already enable TOTP', async () => {
      ...
      expect(responseBody.message).toBeDefined();
      expect(responseBody.message).toBe('Duplo fator de autenticação já habilitado para a conta.');
      expect(responseBody.action).toBeDefined();
      expect(responseBody.action).toBe('Verifique se você está realizando a operação correta.');
      expect(responseBody.error_location_code).toBeDefined();
      expect(responseBody.error_location_code).toBe('CONTROLLER:MFA:TOTP:ENABLE:TOTP_ALREADY_CONFIGURED');
    });
