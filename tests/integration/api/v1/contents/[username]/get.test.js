...
describe('GET /api/v1/contents/[username]', () => {
  describe('Anonymous user', () => {
    ...
    test('"username" existent, but with no "published" "root" content (searching for root content)', async () => {
      const defaultUser = await orchestrator.createUser();
    ...
    test('"username" existent and only with "published" "child" content (searching for root content)', async () => {
      const defaultUser = await orchestrator.createUser();
    ...
    test('"username" existent with 4 contents, but only 2 "root" "published" and strategy "new" (searching for root content)', async () => {
      const defaultUser = await orchestrator.createUser();
    ...
    //CHILD CONTENT
    test('"username" existent, but with no "published" "child" content (searching for child content)', async () => {
      const defaultUser = await orchestrator.createUser();
      ...
      const response = await fetch(
        `${orchestrator.webserverUrl}/api/v1/contents/${defaultUser.username}?content_type=children`
      );
      const responseBody = await response.json();
    ...
    test('"username" existent and only with "published" "root" content (searching for child content)', async () => {
      const defaultUser = await orchestrator.createUser();
    ...
    test('"username" existent with 5 contents, but only 2 "child" "published" and strategy "new" (searching for child content)', async () => {
      const defaultUser = await orchestrator.createUser();
      ...
      const response = await fetch(
        `${orchestrator.webserverUrl}/api/v1/contents/${defaultUser.username}?strategy=new&content_type=children`
      );
      const responseBody = await response.json();
      ...
      expect(responseBody).toStrictEqual([
        {
          ...
          title: 'Quinto contéudo criado',
          body: secondNotRootContent.body,
          status: 'published',
          source_url: null,
    ...
    test('"username" existent with 61 contents, default pagination and strategy "new" (searching for child content)', async () => {
      const defaultUser = await orchestrator.createUser();
