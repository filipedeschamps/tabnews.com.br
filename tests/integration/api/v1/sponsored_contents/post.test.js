...
describe('POST /api/v1/sponsored_contents', () => {
  ...
  describe('User with "create:sponsored_content" feature', () => {
    ...
    test('sponsored content with "source_url" without a protocol', async () => {
      const sponsoredContentsRequestBuilder = new RequestBuilder('/api/v1/sponsored_contents');
      await sponsoredContentsRequestBuilder.buildUser({ with: ['create:sponsored_content'] });
  ...
  test(`sponsored content with "slug" containing more than ${maxSlugLength} bytes`, async () => {
    ...
    expect(responseBody).toStrictEqual({
      id: responseBody.id,
      owner_id: defaultUser.id,
      slug: `this-slug-must-be-changed-from-${1 + maxSlugLength}-to-${maxSlugLength}-bytes`.padEnd(maxSlugLength, 's'),
      title: `Slug with more than ${maxSlugLength} bytes`,
      body: 'Sponsored content body',
      source_url: null,
      created_at: responseBody.created_at,
  ...
  test('active sponsored content from the same user limit reached', async () => {
    ...
    expect(uuidVersion(responseBodyError.error_id)).toBe(4);
    expect(uuidVersion(responseBodyError.request_id)).toBe(4);
  });
